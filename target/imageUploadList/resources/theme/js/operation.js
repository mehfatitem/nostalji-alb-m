...
function edit(datas) {
	...
	swal/*.withForm*/({
		...
		/*formFields: [
		       { id: 'editText', placeholder:'Resim Açıklaması Giriniz...' , value : imgDesc.trim() },
		]*/
	}, function(inputValue) {
		...
		if (inputValue === "") {
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(!data.includes("  Bu açıklama isimli resim sistemde mevcuttur!"))
						$("#"+imgId).text(inputValue);
					var d = new Date();
					var curr_date = putZeroFirst(d.getDate());
					var curr_month = putZeroFirst(parseInt(d.getMonth()) + 1);
					var curr_year = d.getFullYear();
					var curr_hour = putZeroFirst(d.getHours());
					var curr_minutes = putZeroFirst(d.getMinutes());
					var curr_seconds = putZeroFirst(d.getSeconds());
					$("#"+imgId+"date").text(curr_date + "-" + curr_month + "-" + curr_year + " " + curr_hour + ":" + curr_minutes + ":" + curr_seconds);
					setTimeout(function(){
						console.log(data);
						$(".alert").remove();
						if(!data.includes("  Bu açıklama isimli resim sistemde mevcuttur!")){
								...
								success : function(data) {
									if (data == "1") {
										...
										if(length-1  == 0){
											...
											$("#imageTable").remove();
											$("#operations").remove();
											$('body').append("<div class='alert alert-warning' align='center'><strong>Uyarı!</strong> Resim bulunamadı !</div>");
